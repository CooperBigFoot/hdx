...
- `validate(path) -> Result<ValidationReport, _>` / `validate_json(path) -> Result<String, _>` — **conformance**. Runs the spec §14 `MUST` checklist (the 21 ids `M1`–`M6`, `L1`–`L3`, `I1`–`I3`, `H1`–`H2`, `T1`–`T3`, `G1`–`G3`, `Geo1`) over the same shared discovery layer and emits a `ValidationReport` of per-check outcomes — each recording **ran vs skipped**, **pass/fail**, its metadata-deep/byte-deep depth, and a detail/reason — plus an overall `conformant: bool`. It **fails closed** (a violated `MUST` that ran ⇒ non-conformant; a skip never flips the verdict) and honestly reports which checks ran. `L3`, the `M6` axis-regularity leg, and `T2` now run over the realized layout and full time axes; `Geo1` is skipped with a reason only when outlines are optional and absent.
...
- `invalid/irregular-time-axis/` — a fail-closed M6 rule-(b) regression whose scalar and gridded axes remain identical while their interior step is irregular.
- **18 generated invalid trees**: 17 report/entry-gate negatives pin `M2`–`M6`, `L1`/`L2`, `I1`/`I2`, `H1`/`H2`, `T1`, `T3`, or `G2` (the four T3 fixtures cover both declaration sites, calendar, and multi-store aggregation), plus one G3 discovery-error negative. The full classification of all 21 ids lives in `conformance/README.md`.
