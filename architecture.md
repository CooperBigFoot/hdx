...
| Date | Amendment | Why |
|---|---|---|
| 2026-06-02 | **MS9-S1 PyO3 binding macOS-link wiring (the non-obvious build setup, recorded per §8).** `crates/python` is a PyO3 **abi3** extension crate (package `hdx-python`, importable module `hdx`, `pyo3 = "0.24"`, `edition = "2024"`), and three coupled `Cargo.toml` decisions make it link AND test green on Darwin/arm64 under the whole-workspace gate while still shipping a correct wheel. **(1) `[lib] crate-type = ["cdylib", "rlib"]`.** The `cdylib` is the Python extension maturin packages; the `rlib` gives `cargo test`/`clippy --all-targets` a normal Rust library target to build `#[cfg(test)]` unit tests against. A `cdylib`-only crate has **no** buildable unit-test target — the Rust-level link proof here (and MS9-S2's error-mapping unit tests) would be unrealizable. **(2) `extension-module` is an OPTIONAL, NON-DEFAULT feature** (`[features] extension-module = ["pyo3/extension-module"]`), with `pyo3` added `default-features = false`. Enabling `extension-module` unconditionally is the canonical PyO3 macOS gotcha: it tells the linker to leave the `_Py*` symbols **undefined** (resolved by the host interpreter at import time) — correct for the shipped `cdylib`, but it BREAKS linking of every non-cdylib target on macOS (the rlib test executable, doctests) because there is no interpreter to resolve against. So `cargo build/test/clippy --all-targets -- -D warnings` run with the feature **OFF** (rlib/test targets link), and `maturin` turns it **ON** for the wheel only via `[tool.maturin] features = ["extension-module"]` in `crates/python/pyproject.toml` (which also sets `abi3 = true`). **(3) `default-features = false` re-adds `macros` explicitly** (`features = ["abi3-py312", "macros"]`): dropping the default set removes both `extension-module` (wanted gone) and `macros` (still needed for `#[pymodule]`/`#[pyfunction]`/`wrap_pyfunction!`), so `macros` is enumerated back in. **Proof (LOW-1):** `maturin build --interpreter python3.12` produced `target/wheels/hdx-0.1.0-cp312-abi3-macosx_11_0_arm64.whl`, and `python3.12 -c "import hdx; hdx.__core_version()"` in a throwaway venv exited 0. S1 ships exactly one trivial `__core_version()` (the smallest link/import surface); the `validate`/`describe` mirrors land in MS9-S2. `.gitignore` now ignores `crates/python/.venv/`, `crates/python/**/__pycache__/`, and `target/wheels/` (no venv/wheel artifact committed). `hdx-core`/`main.rs` unchanged. | Records the highest-risk MS9 setup so a future agent does not "fix" the build by enabling `extension-module` unconditionally (which silently breaks `cargo test`/`clippy` on macOS) or by dropping the `rlib` type (which deletes the only unit-test target). The two HIGH defects (unconditional `extension-module`; `cdylib`-only) and the LOW import-proof are folded here in one reviewable place, isolated and proven before the verbs are added. |
